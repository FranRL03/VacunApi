...
public interface VacunaRepositorio extends JpaRepository<Vacuna, UUID> {

    @Query("SELECT COUNT(c) FROM Calendario c WHERE c.vacuna.id = ?1")
    int comprobarVacunaEnCalendario(UUID vacunaId);
    ...
    int contarCalendariosConAdministraciones(UUID vacunaId);
